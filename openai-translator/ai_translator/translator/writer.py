...
class Writer:
    ...
    def _save_translated_book_markdown(self, book: Book, output_file_path: str = None):
        ...
        LOG.info(f"pdf_file_path: {book.pdf_file_path}")
        LOG.info(f"pdf_file_book: {book.pages[0].contents}")
        LOG.info(f"开始翻译: {output_file_path}")
    ...
    # def save_translated_book_before_pdf(self, book: Book, output_file_path: str = None):
    #     if output_file_path is None:
    #         output_file_path = book.pdf_file_path.replace('.pdf', f'_beore_translated.pdf')
    #
    #     LOG.info(f"pdf_file_path: {book.pdf_file_path}")
    #     LOG.info(f"开始备份原始文件: {output_file_path}")
    #
    #     # Register Chinese font
    #     font_path = "../fonts/simsun.ttc"  # 请将此路径替换为您的字体文件路径
    #     pdfmetrics.registerFont(TTFont("SimSun", font_path))
    ...
    #     # Save the translated book as a new PDF file
    #     doc.build(story)
    #     LOG.info(f"备份原始文件完成: {output_file_path}")